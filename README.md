...
API Key 有两种配置方式，按需选择其中一种即可：

**方式一：前端设置（推荐）**
1. 打开前端页面
2. 点击左上角聊天面板顶部的齿轮按钮（设置）
3. 在弹出的设置窗口中输入你的 DeepSeek API Key
4. 点击"测速"验证密钥有效性
5. 密钥会保存在浏览器 localStorage 中，下次打开无需重新输入

**方式二：后端配置文件**
```bash
echo "your-deepseek-api-key" > apikey.txt
```
前端未设置密钥时，后端会自动使用 apikey.txt 中的默认密钥。
