...
* pytest（320 项：319 passed + 1 skipped，其中 19 项为 FastAPI TestClient 接口级集成测试）
...
│   └── tests/                   # pytest 测试（320 项：319 passed + 1 skipped）
...
当前收集 320 项：319 项通过，1 项因缺少 GDAL 环境被跳过；其中 19 项为 FastAPI TestClient 发起的接口级集成测试。
...
* **新增图层通道守卫测试**：把「工具必须通过 `_push_layer` / `_register_layer` 操作图层、不得手写 `_pending_layers` / `_registered_layers`」钉成静态约束（AST 扫描）。第九阶段的等高线 bug 属于这一类——症状出现在工具之外（前端不渲染 / 下一个工具读空），单个工具的单测抓不到，只能从源码结构上禁掉；守卫自带自检用例，确保它不是空的

* 测试：新增守卫 2 项；全量 320 项（319 passed + 1 skipped）
...
* **修复填洼逻辑被写反**：`hydrology_analysis` 的"填洼"前置步骤原用 `filled[i,j] > min_nbr` 判定、并把该单元降到最低邻域，等价于反复做 3×3 最小值滤波（形态学腐蚀）——把一个"高原上的洼地" DEM 抹平到全局最低点（实测一个洼地会把周围 48 个单元一起拉低），导致其后的 D8 流向与汇流累积量全部失真。改为"只抬高、绝不降低"的正确填洼（`_fill_sinks`：仅抬升低于最低邻域的洼地单元，边界视为出水口、nodata 视为屏障），并抽成独立可测函数、顺带向量化（原为逐单元双重 Python 循环）

* 测试：新增水文 7 项（平地/单调面不变、洼地被抬高且无残留局部最低、nodata 保留、工具端到端）；全量 315 项（314 passed + 1 skipped）
...
* **修复坡向 / 山体阴影方向整体偏置约 90°**：`dem_analysis` 的坡向原用 `atan2(dzdy, -dzdx)`，
  在北向上栅格（row 增大→南，负 y 分辨率已含在 `dzdy` 里）下与几何真值系统性相差约 90°，
  且山体阴影光源方位因此照错边（本应西北最亮却偏到别处）。用中心凸起锥形 DEM（几何无歧义：
  峰以北下坡必为北=0°、东/南/西=90/180/270°）实测确认，坡向公式改为 `atan2(-dzdx, -dzdy)`，
  实测四向误差 < 4°；山体阴影同步使用同一公式，西北（方位 315°）坡恢复最亮。坡度公式与符号无关，未受影响

* 测试：新增地形分析 3 项（坡向四向回归、西北坡最亮、坡度非零）；全量 305 项（304 passed + 1 skipped）
