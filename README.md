...
基于 Web 的 GIS 数据处理可视化工作台，内置三模型 AI 助手（含免配置 GLM + Agnes 免费模型），通过自然语言交互即可搜索数据、提取建筑轮廓、处理 GIS 数据并加载到地图。支持 `/` 斜杠命令面板一键调用 GIS 工具，chip 标签指示当前启用的技能模块。
...
| 模型 | 能力 |
|------|------|
| **GLM-4.7-Flash+**（默认，免费） | GLM 路由 + GLM 执行，完整 Function Calling，技能文档按需加载 |
| **DeepSeek V4 Flash+** | GLM 路由 + DeepSeek 执行，画图/空间分析时自动参考技能文档 |
| **Agnes 2.0 Flash+**（免费） | GLM 路由 + Agnes 执行，512K 上下文，免费 |

**三模型 + 自校验：** 默认使用 GLM-4.7-Flash+（免费）。内置 Self-Verifying Agent 架构——Agent 1 执行 GIS 操作后，Agent 2 校验结果是否符合用户需求，不通过时自动修正。校验只看用户请求 + 结果摘要，不携带完整工具调用历史，token 开销极低。
...
> ⚠️ **使用前必做：配置 API Key** — 本项目的 AI 功能依赖 DeepSeek / GLM / Agnes 的 API 密钥，请在页面顶部 ⚙️ 齿轮按钮中配置，否则 AI 功能无法使用。GLM 和 Agnes 目前免费。（密钥会由浏览器自动保存）
...
在页面顶部 ⚙️ 齿轮按钮中填入 DeepSeek / GLM / Agnes 的 API 密钥即可，浏览器会自动保存。
...
| 地图引擎 | Leaflet | — |
| 后端 | FastAPI | — |
| **AI 主力** | **GLM-4.7-Flash+（免费）** | [智谱开放平台](https://open.bigmodel.cn/) |
| **AI 备选** | **DeepSeek V4 Flash+** | [DeepSeek 开放平台](https://platform.deepseek.com/) |
| **AI 备选** | **Agnes 2.0 Flash+（免费）** | [Agnes 平台](https://platform.agnes-ai.com/) |
...
```
frontend/             # 前端页面
  index.html          # 主页面（设置弹窗/AI 模型+地理服务分类/图层检查器/流程图弹窗）
  css/style.css       # 样式（暗黑模式/可编辑属性表/列宽拖拽/步骤日志）
  js/                 # JS 模块
    chat.js           # AI 对话（斜杠命令/chip标签/SSE流式/步骤累积折叠/AbortController停止）
    map.js            # 地图（绘制工具/十字准星/热力图/右键菜单/POI/右键DEM）
    layers.js         # 图层面板（拖拽排序/可编辑属性表/筛选/坐标列/列宽拖拽/前后端同步）
    task.js           # 任务管理系统（5状态/localStorage持久化）
    api.js            # API 通信层（图层注册/取消注册/BASE_URL 覆盖）
    upload.js         # 文件上传（每文件独立 AbortController）
    app.js            # 应用初始化（ESC全局关闭/设置弹窗/上下文菜单/检查器）
    time.js           # 时间问候
backend/              # 后端服务
  main.py             # FastAPI 入口（路由/SSE流式/CORS/DataV边界/图层注册/取消注册）
  services/
    ai_service.py     # AI 对话 + SYSTEM_PROMPT + GLM 路由 + 消息构建 + 请求取消
    tools.py          # 19 个 @tool 工具（含 get_elevation DEM/AOI/图层控制/热力图等）
    graph.py          # LangGraph Agent（create_react_agent/SSE流式/取消检查）
    dem_service.py    # 高德高程 API（DEM 单点+网格查询→GeoJSON+WGS-84）
    geo_coords.py     # 坐标转换工具（GCJ-02↔WGS-84/BD-09 互转）
...
skills/               # 按需加载的 Markdown 技能文档
  ...
  amap.md             # 高德 Web API 完整文档（POI/天气/地理编码/坐标转换/高程）
  gdal.md             # GDAL 地理数据处理
...
**2026-07-15**
- 新增 Agnes 2.0 Flash+ 免费模型（agix API），GLM 路由 + Agnes 执行
- Agnes 自路由改为统一用 GLM 路由，省一次跨洋往返
- 为所有 Agnes OpenAI 客户端加 timeout（60s/30s/15s）
- 删除废弃的 gaode AOI 路由（import + endpoints + models）
- 前端模型选择器：三模型排列，Agnes 加「免费」标签
- 模型显示名统一为 Agnes 2.0 Flash+
- 默认模型从 deepseek-routed 改为 glm-routed（免费）
- 处理中加载消息适配 Agnes 模型名
